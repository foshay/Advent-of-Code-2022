...
class monkey:
    ...
    def throw(self):
        ...
        if self.items[0]%self.test == 0:
            ...
            #print("Threw to monkey",self.n1, ":",self.items[0], 'is divisible by', self.test)
            self.items = self.items[1:]
        else:
            self.monkeyFalse.getItems().append(self.items[0])
            #print("Threw to monkey",self.n2, ":",self.items[0], 'is NOT divisible by', self.test)
            self.items = self.items[1:]
    def getItems(self):
        return self.items
    def inspect(self):
        ...
        for i in range(leng):
            ...
            self.items[0] = int(self.getWorry(self.items[0])%9699690)
            #print("\titem after",self.items[0])
            #print('\t\tDiff:', (self.items[0]-before))
            self.throw()
    def getInspect(self):

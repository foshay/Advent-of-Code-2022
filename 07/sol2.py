...
def main():
    ...
    diskSize = 70000000
    spaceNeed = 30000000
    unusedSpace = diskSize - int(root.getAllNodes()[0][0])
    spaceNeed = spaceNeed - unusedSpace
    print("unused space:",unusedSpace,"space Need:", spaceNeed)
    for i in root.getAllNodes()[0]:
        print('-',i)
    print("----------------------------------------")
    print(root.findNodes(spaceNeed))
    ...
    def findNodes(self, target):
        if self.nodes:
            ...
            if self.value <= 100000:
                ...
                val = self.value
            for i in self.nodes:
                test = self.nodes[i].findNodes(target)
                if test >= target and test < min:
                    print('new min',test)
                    min = test
            #print("val",val)
            return min
